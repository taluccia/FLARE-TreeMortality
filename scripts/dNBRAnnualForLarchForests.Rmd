...
# Overview
The code in this script can be copied into Earth Engine to produce dNBR images for Larch forest across eastern Siberia

In EE, the data is processed by fire perimeters and year, to remain within the computational limits of EE.
    
This script is adapted from work mapping fires in eastern Siberia by Talucci et al. 


Below are 4 chunks of JavaScript code that can be copied and pasted into the EE code editor. 

For Chunk #1, Please note the fire perimeters are available for downloaded from the [Arctic Data Center](https://arcticdata.io/catalog/view/doi%3A10.18739%2FA2GB1XJ4M)

For Chunk #2, several lines of code are turn off with only a few turn on. Each variable is either run by itself or in conjunction with one other. This sub-setting was required to remain within the computational limits of EE. The image processing is run on a single fire season. For 2011-2020, the single fire year is subset further by splinting up the perimeters into smaller groups by 'IDobj' for the region that contains the Eastern Siberian Taiga, 
Therefore line of need to be turned off and on in the code to stay within the computational limits of EE

In Chunk #3, define the aoi, so it matches what is run in Chunk #2.

In chunk #4, we process the imagery, where dNBR is calculated from one year pre-fire and one year post-fire. Additionally, the imagery is exported in this chunk and the name of the file will need to be change for each run.

Each run produces a single output for a given year and aoi resulting in XX total runs and image outputs that are then mosaic together, see script XX. 

# Chunk 1
This defines the pre-fir year, year of fire, and post-fire year. These years will need to be changed to iterate through 2001-2020 fire years. This chunk also calls in the fire perimeters, which can be download from the Arctic Data Center. Finally, we create two list with the EcoCode Names for the areas relevant for the project. These list are used to subset the data in Chunk 2, which allowed us to remain within EE computational limits.
```{js}
// Center Map
...
# Chunk 3
Define the aoi, make sure the aoi is defined as the variable run in Chunk 2.
```{js}
print(fc_e.size())
...
# Chunk 5
This chunk exports the dNBR image to assets. The file name should be change for each iteration, anything associated with the aoi 'fc_ne' is consider to be Northeastern Siberia including ecozones Northeast Siberia Taiga (NST), Cherskii–Kolyma mountain tundra (CKMT), and Bering Tunda (BT). For anything associated with the aoi fc_e this is consider eastern Siberia including ecozones Eastern Siberia Taiga (EST), Taimyr-Central Siberian Tundra (TCST), Trans-Baikal Bald Mountain tundra (TBBMT). For additional subsets ofr fc_e add a 1, 2, or 3 depending on the AOI.
```{js}
//------------------------- EXPORT TO ASSET --------------------------------------//
